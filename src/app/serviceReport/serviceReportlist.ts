...
import { Country, Distributor, ProfileReadOnly, ServiceReport, User } from '../_models';
...
import { PreventiveMaintenanceService } from '../_services/preventivemaintenances.service';
...
export class ServiceReportListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.user.isAdmin) {
      ...
      this.role = JSON.parse(sessionStorage.getItem('segments'));
  ...
  getAllRecords() {
    this.serviceReportService.getAll()
      .pipe(first())
      .subscribe((data: any) => this.DataFilter(data.data))
  }
  ...
  DataFilter(data) {
    ...
    //         this.isDist = true;
    //         this.ServiceReportList = data.filter(x => x.serviceRequest.distId == data1.data.defDistId);
    //         break;
  ...
  private createColumnDefs() {
    return [
      ...
      },
      {
        ...
        field: 'serviceRequest.serReqDate',
        filter: true,
        editable: false,
        sortable: true,
    ...
    this.serviceReportService.getView(ServiceReportId)
      ...
      .subscribe({
        next: (data: any) => {
          data = data.data;
          let totalHrs = 0;
          // this.preventiveMaintenanceService.getById(ServiceReportId)
          //   .pipe(first())
          //   .subscribe({
          //     next: (prev: any) => {
          debugger;
          let instrumentName = this.ServiceReportList.find(x=>x.id == ServiceReportId).instrument;
            ...
            .subscribe((serReq: any) => {
              {
                ...
                (data.workTime == null) ? data.workTime = [
                  {
